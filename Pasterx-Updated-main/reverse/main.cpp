...
menu_:
	int choice;
	system("color 3");
	SetConsoleTitleA("Pasterx Updated by .moonstry");
	system("cls");
	printf((" [1] Load Cheat\n [2] Load Driver\n\n > "));
	std::cin >> choice;
	switch (choice)
	...
	case 2:
		...
		system(E("cls"));
		MessageBoxA(NULL, "Loaded", "Updated By .moonstry", MB_OK);
		goto menu_;
		break;
	}
...
void render() {
	...
	if (ShowMenu)
	{
		...
		ImGui::Begin(("Pasterx Updated by .moonstry"), 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
